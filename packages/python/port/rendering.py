...
class PageRenderer:
    ...
    def html_format_retry_page(self):
        text = props.Translatable(
            {
                "en": "It looks like your YouTube data file contains history data in HTML format, but JSON format is required. Please revisit Google Takeout, select YouTube data, click 'Multiple formats', and change History from HTML to JSON. Then download the file again and retry.",
                "de": "Es sieht so aus, als ob Ihre YouTube-Datendatei Verlaufsdaten im HTML-Format enthält, aber das JSON-Format ist erforderlich. Bitte besuchen Sie Google Takeout erneut, wählen Sie YouTube-Daten aus, klicken Sie auf 'Mehrere Formate' und ändern Sie den Verlauf von HTML zu JSON. Laden Sie dann die Datei erneut herunter und versuchen Sie es noch einmal.",
                "it": "Sembra che il tuo file di dati YouTube contenga dati della cronologia in formato HTML, ma è richiesto il formato JSON. Torna su Google Takeout, seleziona i dati YouTube, fai clic su 'Più formati' e cambia la cronologia da HTML a JSON. Quindi scarica nuovamente il file e riprova.",
                "es": "Parece que su archivo de datos de YouTube contiene datos del historial en formato HTML, pero se requiere el formato JSON. Vuelva a Google Takeout, seleccione los datos de YouTube, haga clic en 'Varios formatos' y cambie el historial de HTML a JSON. Luego descargue el archivo de nuevo e inténtelo de nuevo.",
                "nl": "Het lijkt erop dat uw YouTube-gegevensbestand geschiedenis in HTML-formaat bevat, maar het JSON-formaat is vereist. Ga terug naar Google Takeout, selecteer YouTube-gegevens, klik op 'Meerdere indelingen' en wijzig Geschiedenis van HTML naar JSON. Download het bestand opnieuw en probeer het opnieuw.",
                "ro": "Se pare că fișierul dvs. de date YouTube conține date istorice în format HTML, dar este necesar formatul JSON. Reveniți la Google Takeout, selectați datele YouTube, faceți clic pe 'Mai multe formate' și schimbați Istoricul din HTML în JSON. Apoi descărcați din nou fișierul și încercați din nou.",
                "lt": "Atrodo, kad jūsų 'YouTube' duomenų faile istorijos duomenys yra HTML formatu, tačiau reikalingas JSON formatas. Grįžkite į 'Google' išklotinę, pasirinkite 'YouTube' duomenis, spustelėkite 'Keli formatai' ir pakeiskite istoriją iš HTML į JSON. Tada iš naujo atsisiųskite failą ir bandykite dar kartą.",
            }
        )
